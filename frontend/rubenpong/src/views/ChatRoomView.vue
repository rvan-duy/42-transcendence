				...
				<div class="chat-container p-8">
					<header class="chat-header">
						...
						<div style="text-align: right;">
							...
							<span v-if="isOwner">
								<span v-if="chat?.access === 'PUBLIC' || chat?.access === 'PROTECTED'"
									class="btn px-2 py-1 text-xs m-1 bg-blue-500 hover:bg-blue-300 text-white"
									@click="isVisible = true"> Set
									/ Change Password</span>
								<Modal v-model:visible="isVisible" class="text-black" style="text-align: left;"
									:cancel-button="cancelBtn" :ok-button="okBtn" :title="'Set Password'">
									<div>
										<label>This will make sure the channel cannot be entered without the correct
											password.</label>
										<span class="text-black pr-4"><input v-model="newPassword" VALYE type="text"
												name="username" placeholder="Enter password..." required
					...
					<main class="chat-main">
						<div class="chat-sidebar">
							...
							<div v-if="isAdmin">
								...
								<span class="text-black pr-4">
									<input v-model="input" type="text" placeholder="Search users..." VALYE required
										style="border-radius: 20px; width:300px; font-size: 12px; height: 35px;">
									<!-- {{ filteredList() }} -->
									<div v-for="user in filteredList()" :key="user.id" :value="user">
										<span><button
										...
										<span>
											<button v-if="!usersAdded.find(el => el.id === user.id)"
												class="bg-blue-500 text-white text-xs py-1 px-1 rounded-full m-1"
												@click="addUser(user)">Add</button></span>
									</div>

									<div v-if="input && !allUsers.length" class="item error" style="text-align: center;">
							...
							<ul id="users">
								<li v-for="user in usersAdded" :key="user.id" class="m-1">
									<span
										@click="user.id === idUser ? goTo('user') : goTo('otheruser/' + user.name + '?id=' + user.id)">
										<img :src="String(getUserPicture(user.id))" width="30" height="30"
									...
									<span v-if="determineAdmin(user.id)" class="text-green-200 text-xs p-1">
										Admin
									</span>
									<span v-else-if="user.id !== chat?.ownerId && idUser === chat?.ownerId"
										class="text-green-200 text-xs p-1">
										<button
											class="bg-green-400 hover:bg-green-500 text-white text-xs py-1 px-1 rounded-full m-1"
											@click="confirmAndGo('make ' + user.name + ' Admin', makeAdmin, user.id)">Make
											Admin</button>
									</span>
						...
						<div class="chat-messages">
							<div v-for="message in messages" id="message-list" ref="messageContainer" :key="message.id"
								class="message">
								<p class="meta">
									{{ message.username }} <span>{{ toLocale(message.timestamp) }}</span>
								</p>
								<p v-if="message.invite === false" class="text">
									<!-- if the message is not an invite -->
									{{ message.body }}
								</p>
								<p v-else-if="message.invite === true && message.authorId === idUser" class="text">
								...
								<button v-if="message.invite === true && message.authorId !== idUser"
									class="bg-blue-500 border border-red-500 hover:bg-red-400 text-white py-1 px-2 rounded-full text-xs m-3"
