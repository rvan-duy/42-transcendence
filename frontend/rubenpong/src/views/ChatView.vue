				...
				<main class="join-main">
					<form @submit.prevent>
						<div v-if="chatCreate === false">
							<div class="form-control">
								...
								<select id="room" v-model="selectedChat" class="text-black" name="room"
							...
							<div v-if="selectedChat?.access === 'PROTECTED' && !myRooms.includes(selectedChat)">
								...
								<span class="text-black pr-4"><input id="username" v-model="enteredPW" VALYE type="text"
							...
							<button
								...
								type="submit" class="btn bg-blue-100">
								Join Chat
							</button>
							<button v-else type="submit" class="btn bg-blue-100"
					...
					<form @submit.prevent>
						<div v-if="chatCreate === true" class="form-control">
							...
							<select id="chat_access" v-model="newChat.access" class="text-black" name="room"
								style="border-radius: 20px" required>
							...
							<span class="text-black pr-4"><input id="chatName" v-model="newChat.name" VALYE type="text"
									name="username" placeholder="Enter a name for the chat" required
									style="border-radius: 20px; width:300px; font-size: 12px; height: 35px;"> </span>
							<div v-if="newChat.access === 'PROTECTED'">
								<label for="name" class="pt-2">Password</label>
								<span class="text-black pr-4"><input id="password" v-model="newChat.password" VALYE
										type="text" name="username" placeholder="Enter a password for the chat" required
										style="border-radius: 20px; width:300px; font-size: 12px; height: 35px;"> </span>
							</div>
							{{ newChat }}
							...
							<button v-else type="submit" class="btn bg-blue-100" @click="createChat(newChat)">
								Create Chat
							</button>
						</div>
					</form>
